...
int bpapi_msg_validate (cJSON *json) {
    ...
    if(itm == NULL || cJSON_IsNumber(json) == cJSON_False) {
        printf("[bpapi_msg_validate] Field 'cmd' missing from message\n");
        return 1;
    }
    // Check `reqid`
    itm = cJSON_GetObjectItem(json, "reqid");
    if(itm == NULL || cJSON_IsNumber(json) == cJSON_False) {
        printf("[bpapi_msg_validate] Field 'reqid' missing from message\n");
        return 1;
    }
...
int bpapi_msg_AUTH (struct BPApiClient *client, cJSON *json, cJSON *resp) {
    int err = 0;
    err = strcmp((const char *)cJSON_GetObjectItem(json, "key")->valuestring, BPAPI_KEY);
...
int bpapi_msg_DEVICES (struct BPApiClient *client, cJSON *json, cJSON *resp) {
    ...
    for(int i = 0; i < BP_DEVICE_ALLOC_SIZE; i++) {
        if(device_list[i] != NULL) 
            cJSON_AddItemToArray(resp_devices, bpdev_to_json(device_list[i]));
    }
...
int bpapi_msg_ZMK_CONTROL_WRITE (struct BPApiClient *client, cJSON *json, cJSON *resp) {
    ...
    if(device_object == NULL || save == NULL || hex == NULL || field == NULL) {
        // Fail
        printf("[bpapi_msg_ZMK_CONTROL_WRITE] FAIL: 'device'/'save'/'bytes'/'field' field missing from request\n");
    ...
    struct BPDev *device = find_device(NULL, serial_buff, NULL);

    // Check device
    if(device == NULL) {
        // Fail
        printf("[bpapi_msg_SET_MOUSE_SENSITIVITY] FAIL: could not find device %ls\n", serial_buff);
        cJSON_AddBoolToObject(resp, "status", cJSON_False);
        return 1;
    }
    ...
    if(bytes_len <= 0) {
        // Fail
        printf("[bpapi_msg_ZMK_CONTROL_WRITE] FAIL: parse bytes\n");
        cJSON_AddBoolToObject(resp, "status", cJSON_False);
        return 1;
    }
    ...
    if(err < 0) {
        // Fail
        printf("[bpapi_msg_ZMK_CONTROL_WRITE] FAIL: could not write\n");
        cJSON_AddBoolToObject(resp, "status", cJSON_False);
        return 1;
    }
...
int bpapi_msg_ZMK_CONTROL_READ (struct BPApiClient *client, cJSON *json, cJSON *resp) {
    ...
    if(device_object == NULL || field == NULL) {
        // Fail
        printf("[bpapi_msg_ZMK_CONTROL_READ] FAIL: 'device'/'field' field missing from request\n");
        cJSON_AddBoolToObject(resp, "status", cJSON_False);
        return 1;
    }
    // Find device
    ...
    if(device == NULL) {
        // Fail
        printf("[bpapi_msg_ZMK_CONTROL_READ] FAIL: could not find device %ls\n", serial_buff);
        cJSON_AddBoolToObject(resp, "status", cJSON_False);
        return 1;
    }
    ...
    if(rlen <= 0) {
        // Fail
        printf("[bpapi_msg_ZMK_CONTROL_READ] FAIL: could not read\n");
        cJSON_AddBoolToObject(resp, "status", cJSON_False);
        return 1;
    }
